                             ...
                             u_char *bin, u_int32_t bin_size, void *user_data)
{
    ...
    // printf("DEBUG:load_icode_mapper(0x%x,%d,0x%x,%d,user_data)\n", va, sgsize, bin, bin_size);
    for (i = 0; i < bin_size; i += BY2PG)
    {
        ...
        if (i == 0)
        {
            ...
            {
                bcopy(bin, (char *)page2kva(p) + offset, bin_size);
            }
            else
            {
