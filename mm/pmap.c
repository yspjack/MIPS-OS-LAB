...
static Pte *boot_pgdir_walk(Pde *pgdir, u_long va, int create)
{
    ...
    if (!(*pgdir_entryp & PTE_V) && create)
    {
        // create one
        struct Page *page;
        page = alloc(BY2PG, BY2PG, 1);
        *pgdir_entryp = PADDR(page) | PTE_V;
        // set the correct permission bits
    }
    /* Step 3: Get the page table entry for `va`, and return it. */
    pgtable = KADDR(PDE_ADDR(*pgdir_entryp));
...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
    ...
    int n = size / BY2PG;
    for (i = 0; i < n; ++i, va += BY2PG, pa += BY2PG)
    {
        pgtable_entry = boot_pgdir_walk(pgdir, va, 1);
        *pgtable_entry = PTE_ADDR(pa) | PTE_V | perm;
    }
}
